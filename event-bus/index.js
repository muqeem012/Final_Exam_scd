...
app.post('/events', async (req, res) => {
    ...
    await axios.post(`${process.env.AUTH_SERVICE}/events`, event).catch((err) => {
        console.log('Auth Service: ', err.message);
    });

    await axios.post(`${process.env.CLASSROOM_SERVICE}/events`, event).catch((err) => {
        console.log('Classroom Service: ', err.message);
    });

    await axios.post(`${process.env.POST_SERVICE}/events`, event).catch((err) => {
